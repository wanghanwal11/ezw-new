  ...
  <div style="height: 100%" :style="{overflow:overflow}" class="hjzx">
    ...
    <div @touchmove.prevent style="width: 100%;position: fixed;z-index: 99;top: 0;">
        <x-header style="background: #2989ff;width: 100%;margin-top: 25px">
          ...
          <div style="color: white">呼叫中心</div>
        </x-header>
    ...
    <scroller :on-infinite="infinite" ref="my_scroller" refreshText>
      ...
      <div v-for="(item, index) in items1" @click="hjzxxq(item)" :key="index" style="background: white;padding: 15px;display: -webkit-box;border-bottom: 1px solid #ccc">
        <div style="-webkit-box-flex: 1;padding: 0px 10px">
          ...
          <div style="font-size: 14px;line-height: 25px;color: #999999">来源:{{item.type}}</div>
      ...
      <popup v-model="show9" position="right" @on-hide="onhide"  style="background: white">
        ...
        <div style="width:300px;">
          <div style="padding: 10px">
            ...
            <div>
              <div  style="margin: 8px 0">紧急程度</div>
            ...
            <!--</div>-->
            <div>
            <div  style="margin: 8px 0">事件状态</div>
            <btnlist :items="btnlists1"></btnlist>
          </div>
            <div>
              <div  style="margin: 8px 0">事件类型</div>
              <btnlist :items="btnlists2"></btnlist>
            </div>
            <div>
              <div  style="margin: 8px 0">事件大类</div>
              <btnlist :items="btnlists3"></btnlist>
            </div>
            <div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        loading:false,
        titlecontent:'',
        items:[{title:"紧急程度",type:"sjlx",check:false},{title:"事件状态",type:"sjzt",check:false}],
        btnlists:[{name:"一般",show:false},{name:"紧急",show:false}],
        btnlists1:[{name:"已受理",show:false},{name:"已转办",show:false},{name:"已回复",show:false},{name:"已办结",show:false}],
        btnlists2:[{name:"咨询",show:false},{name:"求助",show:false},{name:"意见",show:false},{name:"感谢",show:false},{name:"投诉",show:false},{name:"建议",show:false},{name:"举报",show:false},{name:"信访",show:false},{name:"其他",show:false}],
        btnlists3:[{name:"国土资源",show:false},{name:"经济综合",show:false},{name:"信息产业",show:false},{name:"宣传舆论",show:false},{name:"政法",show:false},{name:"政治综合",show:false},{name:"城市经济",show:false},{name:"城市建设管理",show:false},{name:"居民生活",show:false},{name:"劳动保障",show:false},{name:"三农问题",show:false},{name:"社会事业",show:false},{name:"行政效能",show:false},{name:"其他问题",show:false}],
      }
    },
    ...
    methods: {
        load(back){
          ...
          for (let value of vm.btnlists2) {
            if(value.show){
              eventtypename.push(value.name)
            }
          }
          for (let value of vm.btnlists3) {
            if(value.show){
              bigtypename.push(value.name)
            }
          }
          ...
          eventtypename=eventtypename.join(',')
          bigtypename=bigtypename.join(',')
          var reg=/,$/gi;
          lev=lev.replace(reg,"");
          st=st.replace(reg,"");
          eventtypename=eventtypename.replace(reg,"");
          bigtypename=bigtypename.replace(reg,"");
          vm.$ajax("work_list",{
            type:'呼叫中心',
            page:vm.page,
            rows:'10',
